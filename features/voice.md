...
* 英语口语练习，作为一门独立的课程来开发，跟之前的英语区分开
* 因为实时语音大模型的reasoning能力可能较弱，在设计prompt的时候，可以把prompt设计的简介一些，教学目标以PET的口语内容为主

## 交互设计

* 因为是纯语音交互，请设计成跟chatgpt实时交互类似，类似打电话的交互模式
* 同时在屏幕上展示文字作为辅助
* 进入口语教学课程，就直接进入”实时交互“模式，跟前的chat模式区分开，并且follow之前的设计，主动跟学生打招呼


## 系统设计

* 以火山的实时语音大模型为核心模型，可以接受和输出语音，也能评价学生口音的正确性, api key可以通过环境变量VOLC_AUDIO_API_KEY获取


##  volc model for audio interaction
